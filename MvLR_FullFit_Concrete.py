...
def main():
    ...
    additionalinfo = "Multivariate Linear Regression run on " + dateandtime + "FULL FIT"

    # Create a .txt file to store the output in:
    output_file = open("output_file.txt",'w')
    numberOfTestsstring = "\n---------------------------------------------------------------------------\n" \
                          "Name of input file: " + filename + "\n" \
                          + additionalinfo + "(Ignore percent used for testing)""\n" \
    ...
    for i in range(1, numyvariables):
        ...
        # save the figure
        titlestring = yvariablenames[i] + "_" + "Full_Fit" + ".png"
